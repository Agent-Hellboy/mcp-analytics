...
NAMESPACE=${NAMESPACE:-mcp-analytics}
GATEWAY_PORT=${GATEWAY_PORT:-8083}
API_KEY=${API_KEY:-changeme}
REQ_START=${REQ_START:-20}
REQ_END=${REQ_END:-230}
KIND_CLUSTER_NAME=${KIND_CLUSTER_NAME:-mcp-analytics}
...
docker build -t mcp-analytics-api:latest services/api
docker build -t mcp-analytics-ingest:latest services/ingest
docker build -t mcp-analytics-processor:latest services/processor
docker build -t mcp-analytics-ui:latest services/ui
docker build -t mcp-analytics-mcp-server:latest services/mcp-server
docker build -t mcp-analytics-mcp-proxy:latest services/mcp-proxy

kind load docker-image mcp-analytics-api:latest --name "$KIND_CLUSTER_NAME"
kind load docker-image mcp-analytics-ingest:latest --name "$KIND_CLUSTER_NAME"
kind load docker-image mcp-analytics-processor:latest --name "$KIND_CLUSTER_NAME"
kind load docker-image mcp-analytics-ui:latest --name "$KIND_CLUSTER_NAME"
kind load docker-image mcp-analytics-mcp-server:latest --name "$KIND_CLUSTER_NAME"
kind load docker-image mcp-analytics-mcp-proxy:latest --name "$KIND_CLUSTER_NAME"

kubectl apply -f k8s

kubectl -n "$NAMESPACE" rollout restart deployment/mcp-analytics-api
kubectl -n "$NAMESPACE" rollout restart deployment/mcp-analytics-ingest
kubectl -n "$NAMESPACE" rollout restart deployment/mcp-analytics-processor
kubectl -n "$NAMESPACE" rollout restart deployment/mcp-analytics-ui
kubectl -n "$NAMESPACE" rollout restart deployment/mcp-analytics-gateway
...
kubectl -n "$NAMESPACE" rollout status deployment/mcp-analytics-ui --timeout=180s
kubectl -n "$NAMESPACE" rollout status deployment/mcp-analytics-gateway --timeout=180s
...
PIDS=()
kubectl -n "$NAMESPACE" port-forward svc/mcp-analytics-gateway "${GATEWAY_PORT}:8083" >/tmp/mcp-pf.log 2>&1 &
PIDS+=("$!")
...
try:
    jobs = ["mcp-analytics-api", "mcp-analytics-ingest", "mcp-analytics-processor"]
    up_values = []
    for job in jobs:
